...
# --- TELEMETRY LAYOUT CONFIGURATION ---
LOG_FILE_PATH = "data/production_logs.csv"

# Initialize production logs file with appropriate feature headers if it does not exist
os.makedirs("data", exist_ok=True)
if not os.path.exists(LOG_FILE_PATH):
    with open(LOG_FILE_PATH, mode="w", newline="") as f:
        writer = csv.writer(f)
        headers = [f"V{i}" for i in range(1, 29)] + [
            "scaled_amount",
            "scaled_time",
            "prediction",
        ]
...
def predict_transaction_fraud(transaction: Transaction):
    """Accepts a feature vector, logs it, and returns a fraud risk assessment."""
    ...
    try:
        ...
        # --- TELEMETRY LOGGING STEP ---
        with open(LOG_FILE_PATH, mode="a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(transaction.features + [prediction])
